"""Archive input reading and inspection."""
...
from .ecospold1_reader import index_ecospold_archive, load_ecospold_archive
from .lcia import (
...
def merge_unit_registries(*registries: dict[str, UnitInfo]) -> dict[str, UnitInfo]:
    merged: dict[str, UnitInfo] = {}
    for registry in registries:
        for unit_id, incoming in registry.items():
            existing = merged.get(unit_id)
            if existing is None:
                merged[unit_id] = incoming
                continue
            merged[unit_id] = UnitInfo(
                unit_id=existing.unit_id,
                name=_prefer_value(existing.name, incoming.name),
                group_id=_prefer_value(existing.group_id, incoming.group_id),
                raw=existing.raw if existing.raw else incoming.raw,
                group_name=_prefer_value(existing.group_name, incoming.group_name),
                conversion_factor=(
                    existing.conversion_factor
                    if existing.conversion_factor is not None
                    else incoming.conversion_factor
                ),
                is_reference_unit=existing.is_reference_unit or incoming.is_reference_unit,
                flow_property_id=_prefer_value(existing.flow_property_id, incoming.flow_property_id),
                flow_property_name=_prefer_value(existing.flow_property_name, incoming.flow_property_name),
            )
    return merged
...
def _iter_source_entries(source: str) -> Iterator[tuple[str, bytes]]:
    ...
    if path.suffix.lower() in ARCHIVE_SUFFIXES:
        ...
        return
    if path.suffix.lower() in {".xml", ".spold"}:
        yield path.name, path.read_bytes()
        return
    raise DataFormatError(
        f"Unsupported input source: {source}. Supported inputs are JSON-LD folders, EcoSpold1 folders, .zip archives, .zolca archives, .xml files, and .spold files."
    )
...
def _identifier_aliases(value: object) -> tuple[str, ...]:
...
def _build_category_path_lookup(entries: list[DatasetEntry]) -> tuple[dict[str, str], CategoryPathDiagnostics]:
    category_entries: dict[str, DatasetEntry] = {}
    alias_to_category_id: dict[str, str] = {}
    for entry in entries:
        if entry.object_type != "category":
            continue
        category_entries.setdefault(entry.object_id, entry)
        for alias in _identifier_aliases(entry.data):
            alias_to_category_id.setdefault(alias, entry.object_id)
        alias_to_category_id.setdefault(entry.object_id, entry.object_id)
    if not category_entries:
        return {}, CategoryPathDiagnostics()
    cache: dict[str, str] = {}
    visiting: set[str] = set()

    def resolve(category_id: str) -> str:
        canonical_id = alias_to_category_id.get(category_id, category_id)
        if canonical_id in cache:
            return cache[canonical_id]
        if canonical_id in visiting:
            return ""
        entry = category_entries.get(canonical_id)
        if entry is None:
            return ""
        ...
        cache[canonical_id] = path
        return path

    for category_id in category_entries:
        resolve(category_id)
    lookup = {alias: resolve(category_id) for alias, category_id in alias_to_category_id.items()}
    for category_id in category_entries:
        lookup.setdefault(category_id, resolve(category_id))
    diagnostics = CategoryPathDiagnostics(
        n_category_objects_indexed=len(category_entries),
        n_category_identifier_aliases_indexed=len(alias_to_category_id),
        n_category_paths_resolved=sum(1 for category_id in category_entries if resolve(category_id)),
    )
...
def _flow_info(entry: DatasetEntry, category_path_lookup: Optional[dict[str, str]] = None) -> FlowInfo:
    ...
    if category_path_lookup:
        for category_ref_id in _identifier_aliases(category):
            resolved_category_path = category_path_lookup.get(category_ref_id, "")
            if resolved_category_path:
                break
    ...
    if reference_flow_property_id is None:
        for flow_property_factor in entry.data.get("flowProperties", []) or []:
            ...
            if not (
                ...
                or flow_property_factor.get("isRefFlowProperty")
...
def _build_flow_lookup(entries: list[DatasetEntry]) -> tuple[dict[str, FlowInfo], CategoryPathDiagnostics]:
    flow_entries = [entry for entry in entries if entry.object_type == "flow"]
    category_path_lookup, diagnostics = _build_category_path_lookup(entries)
    flows = {entry.object_id: _flow_info(entry, category_path_lookup) for entry in flow_entries}
    elementary_count = 0
    elementary_with_category_path = 0
    unresolved_sample: list[dict[str, object]] = []
    flows_with_category_reference = 0
    flows_with_resolved_category_reference = 0
    for entry in flow_entries:
        flow = flows[entry.object_id]
        category_aliases = _identifier_aliases(entry.data.get("category"))
        if category_aliases:
            flows_with_category_reference += 1
            if any(category_path_lookup.get(alias, "") for alias in category_aliases):
                flows_with_resolved_category_reference += 1
        if not flow.is_elementary:
            continue
        elementary_count += 1
        if flow.category_path.strip():
            elementary_with_category_path += 1
            continue
        ...
        unresolved_sample.append(
            {
                ...
                "source_file": entry.path,
                "category": entry.data.get("category"),
                "category_reference_aliases": list(category_aliases),
                "raw_keys": sorted(str(key) for key in entry.data.keys()),
            }
        )
    diagnostics.n_flows_with_category_reference = flows_with_category_reference
    diagnostics.n_flows_with_resolved_category_reference = flows_with_resolved_category_reference
    diagnostics.n_elementary_flows = elementary_count
    diagnostics.n_elementary_flows_with_category_path = elementary_with_category_path
    diagnostics.n_unresolved_elementary_flows = elementary_count - elementary_with_category_path
    diagnostics.pct_elementary_flows_with_category_path = (
        (100.0 * elementary_with_category_path / elementary_count) if elementary_count else 0.0
    )
    diagnostics.sample_unresolved_elementary_flows = unresolved_sample
...
def _build_archive_index(source: str) -> tuple[JsonLdArchiveIndex, list[str]]:
    ...
    for rel_path, raw_bytes in _iter_source_entries(source):
        ...
        object_id = extract_object_id(data) or rel_path
        name = extract_name(data)
        # The archive index only needs parsed metadata for non-process objects.
        # Keeping the original raw JSON bytes here duplicates a large fraction
        # of the input archive in memory during priority/reduction workflows.
        entry = DatasetEntry(
            path=rel_path,
            raw_bytes=b"",
            data=data,
    ...
    flows, category_path_diagnostics = _build_flow_lookup([*flow_entries, *category_object_entries])
    ...
    return (
        JsonLdArchiveIndex(
            ...
            category_path_diagnostics=category_path_diagnostics,
        ),
        entry_names,
    )
...
def index_archive(
    ...
    *,
    ...
    ecospold_xml_error_policy: str = "fail",
) -> JsonLdArchiveIndex:
    ...
    if not index.processes and not index.flows and not index.impact_categories:
        ...
        elif any(name.lower().endswith((".xml", ".spold")) for name in entry_names):
            index = index_ecospold_archive(
                source,
                require_processes=require_processes,
                require_flows=require_flows,
                xml_error_policy=ecospold_xml_error_policy,
            )
...
def load_archive(
    ...
    *,
    ...
    flows, category_path_diagnostics = _build_flow_lookup(entries)
